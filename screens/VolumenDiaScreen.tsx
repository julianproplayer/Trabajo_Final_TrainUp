// VolumenDiaScreen.tsx
import React, { useEffect } from 'react';
import { View, Text, StyleSheet, BackHandler, FlatList, Image } from 'react-native';
import { StackNavigationProp } from '@react-navigation/stack';
import { RouteProp } from '@react-navigation/native';
import { RootStackParamList } from '../navigation';

type VolumenDiaScreenNavigationProp = StackNavigationProp<RootStackParamList, 'VolumenDia'>;
type VolumenDiaScreenRouteProp = RouteProp<RootStackParamList, 'VolumenDia'>;

type Props = {
  navigation: VolumenDiaScreenNavigationProp;
  route: VolumenDiaScreenRouteProp;
};

// Mapa de imágenes locales
const imageMap: Record<string, any> = {
  press_banca: require("../assets/press_de_banca.png"),

};

export default function VolumenDiaScreen({ navigation, route }: Props) {
  const { day, exercises } = route.params;

  useEffect(() => {
    const backAction = () => {
      navigation.goBack();
      return true;
    };
    const backHandler = BackHandler.addEventListener("hardwareBackPress", backAction);
    return () => backHandler.remove();
  }, [navigation]);

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Rutina de {day} 💪</Text>
      <FlatList
        data={exercises}
        keyExtractor={item => item.id}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.exercise}>{item.title}</Text>
            <Text style={styles.description}>{item.description}</Text>
            {imageMap[item.id] && (
              <Image
                source={imageMap[item.id]}
                style={styles.exerciseImage}
                resizeMode="contain"
              />
            )}
          </View>
        )}
      />
    </View>
  );
}

const styles = StyleSheet.create({
  container: { flex:1, padding:20, backgroundColor:"#fff" },
  title: { fontSize:22, fontWeight:"bold", marginBottom:20, textAlign:"center" },
  card: { marginBottom:15, padding:15, borderWidth:1, borderRadius:8, borderColor:"#ccc" },
  exercise: { fontSize:18, fontWeight:"600", marginBottom:5 },
  description: { fontSize:14, color:"#333" },
  exerciseImage: { width: 250, height: 250, marginTop: 10, alignSelf: "center" }
});
